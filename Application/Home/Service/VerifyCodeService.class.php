...
/**
 * 抄thinkphp3.2的Verfy.class.php
 * 加了参数  config['fontDoc'] 控制输出的文字和字母的字体。在config['fontDoc']下还有两个目录，一个ttfs【英文字体】，一个zhttfs【中文字体】
 *加了参数  config['useImgDoc'] 背景图目录
 *
 *
...
class VerifyCodeService
{
    protected $config =	array(
        ...
        'imageW'    =>  0,               // 验证码图片宽度
        'length'    =>  5,               // 验证码位数
        'fontDoc'   =>  '',              //存放字体的目录(物理路径)
        'fontttf'   =>  '',              // 验证码字体，不设置随机获取（字体的文件名，不含目录，如【中华字体.ttf】）
        'useImgBg'  =>  false,           // 使用背景图片(物理路径)（=====当前不能指定某个图片，建议后期改这个字段，false表示不开启，其他就用图片名称即可）
        'useImgDoc' =>  '',              // 使用背景图片的目录(物理路径)
    ...
    public function entry($id = '') {
        ...
        // 验证码使用随机字体
        $ttfPath = $this ->fontDoc . ($this->useZh ? 'zhttfs' : 'ttfs') . '/';
    ...
    private function _background() {
        $path = $this->useImgDoc;//dirname(__FILE__).'/Verify/bgs/';
        $dir = dir($path);
